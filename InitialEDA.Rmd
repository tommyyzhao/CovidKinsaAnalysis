...
## Guiding Question
Can we make predictions or find useful correlations in Covid data using [Kinsa's](https://www.kinsahealth.co/enterprise/kinsa-insights/) health data?
...
Below, we load the most recent nytimes data directly from their Github. As of 4/7/2020, we have 38197 rows in the data frame.
...
Let's see how Covid cases in New York City evolved:
...
```
This is consistent with the news so far... let's see what the Kinsa data looks like for New York County.
```{r}
Kinsa_NY <- KinsaData %>% 
...
```
A side-by-side view will allow us to compare the two better
```{r}
require(gridExtra)
...
```
The anomaly_diff seems to peak around the start of the Covid cases being recorded, so there is clearly some sort of correlation going on here. For the future, further statistical analysis will be needed, as well as a country-wide approach to determine the total "effectiveness" of Kinsa's data in predicting Covid. NYC could be an outlier after all.
